...
def detect_weak_topics(user_id, app=None):
    ...
    def build_area(name, items):
        ...
        return {"topic": name, "category": name, "questions": len(items), "attempts": len(items), "interviews": len({item.get("interview_id") for item in items}), "average_score": average, "average_relevance": _average([_score(item.get("relevance_score")) for item in items if _score(item.get("relevance_score")) is not None]), "average_correctness": _average([_score(item.get("correctness_score")) for item in items if _score(item.get("correctness_score")) is not None]), "average_completeness": _average([_score(item.get("completeness_score")) for item in items if _score(item.get("completeness_score")) is not None]), "average_communication": _average([_score(item.get("communication_score")) for item in items if _score(item.get("communication_score")) is not None]), "status": status, "trend": _trend(_average(previous), _average(recent)), "previous_average": _average(previous), "recent_average": _average(recent), "missing_concepts": _concepts(items)}
